...
int main(int argc, char** argv)
{
    ...
    if (argc == 3) {
        ...
        model_name = (char*) "./model/yolov5s-seg.rknn";
    }

    rknn_app_context_t rknn_app_ctx;
    memset(&rknn_app_ctx, 0, sizeof(rknn_app_context_t));

    ret = init_yolov5_seg_model(model_name, &rknn_app_ctx);
    if (ret != 0)
    {
        printf("init_yolov5_seg_model fail! ret=%d model_name=%s\n", ret, model_name);
        return -1;
    }
    ...
    while(1){
        ...
        inference_yolov5_seg_model(&rknn_app_ctx, &orig_img, &od_results);
...
//      imwrite("./out.jpg", orig_img);
    ...
    release_yolov5_seg_model(&rknn_app_ctx);
